...
- **실제 사용된 lucide 아이콘 (23종)** 과 기본 size(px):
...
- **구성(위→아래):**
  ...
  2. **제목** `<input class="detail-title" placeholder="제목 없음">` (34/700, -0.02em, lh 1.25, margin-bottom 10px), value=`post.title`, onChange → `updatePost(id,{title})`.
  3. **메타** `.detail-meta`(gap 7px, margin-bottom 22px, color `--text-muted`): `Calendar size={14}` + `<span>{formatDate(post.createdAt)} 작성</span>` + `.detail-meta__dot`(3×3 원, bg `--gray-300`) + `<span>자동 저장됨</span>`.
  4. **본문** `<textarea class="detail-content" placeholder="내용을 입력하세요. 떠오르는 생각, 할 일, 메모를 자유롭게 기록해 보세요.">` (15/1.75, `--text-body`, min-height 340px, `resize:none`), value=`post.content`, onChange → `updatePost(id,{content})`.
  5. **글자 수 칩** `<CharCount text={post.content} />` → `.detail-charcount`(§2.7.16), 본문 다음 마지막 자식. `post.content`만 전달(제목 제외, FR-008), grapheme 수를 우측 하단 sticky로 실시간 표시. 빈 내용 → `0자`.
- **상호작용:**
...
- **reduced-motion / 다크모드:** **미구현**. `prefers-reduced-motion`, `prefers-color-scheme` 대응 없음(라이트 단일 테마).
  > TODO: 확인 필요 — 다크모드/reduced-motion은 현재 코드에 없음. 향후 요구되면 토큰 재매핑 필요.
...
> **미사용(정의만) 토큰 목록**(보존용): `--gray-25`, `--gray-700`(직접 참조 없음, 램프값으로만), `--blue-200`, `--blue-300`, `--blue-400`, `--gold-300`, `--surface-inverse`, `--accent-soft-fg`, `--status-warning`(및 그 참조원 `--amber-500`), `--radius-xs`, `--dur-normal`, `--dur-slow`. — 삭제하지 말 것(디자인 시스템 원본에서 이식된 완전 램프/스케일의 일부).
