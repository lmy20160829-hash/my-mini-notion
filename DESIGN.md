...
- **상태(`.is-active`):** 마크·정렬·목록·링크 버튼은 `.icon-btn.is-active`(`aria-pressed`도 함께 부여) — bg `--surface-active`, color `--text-strong`(§2.10 `.fmt-bar`와 동일 값 재사용). **주의(B1 Minor 정리):** 기존 `.fmt-bar .icon-btn.is-active` 규칙(globals.css:866)은 `.fmt-bar` 안에서만 적용돼 상단 툴바에서는 하이라이트가 먹지 않는다 — `.top-toolbar .icon-btn.is-active` 규칙을 별도로 추가해 해결했다.
