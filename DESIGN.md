...
- **구성 순서:**
  ...
  5. `.sidebar__scroll`:
     ...
     - `SidebarSection label="앱"`: `SidebarItem 캘린더(Calendar)`, `할 일(SquareCheck)`, `휴지통(Trash2)` — 셋 다 **`disabled`(아직 만들지 않은 기능)**. `is-disabled` + `aria-disabled` + 툴팁 "…(준비 중)"(§2.5). 실제 구현 후보는 `docs/BACKLOG.md` 참조.
...
| 섹션 빈 상태 | `아직 글이 없어요.` |
| 섹션 "앱" + 항목 | `앱` / `캘린더` / `할 일` / `휴지통` (보이는 레이블). 툴팁은 `캘린더 (준비 중)` 등 — 3종 모두 미구현(§3.3) |
...
- **disabled:** 두 갈래다.
  ...
  - **아직 만들지 않은 셸 항목**(상단바 5종 + 사이드바 "앱" 3종, 총 8개)은 `aria-disabled` + `.is-disabled` — `cursor:default; opacity:0.45`, 툴팁에 "(준비 중)". `disabled` 속성을 쓰면 툴팁이 사라져 상태를 알릴 수단이 없어지기 때문이다(§2.2).
...
| `components/AppShell.tsx` | 셸 구성·네비·인증 가드·검색·아이콘·사이드바 접기 토글·미구현 8종 비활성 표시 | §3, §3.3.1, §1.7 |
