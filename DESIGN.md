...
- 렌더 가드: `if (!auth.ready || !auth.session || !app.loaded) return null;` — 인증 확정 전/비로그인/글 로드 전에는 셸 자체를 그리지 않음.
...
### 4.1 로그인 `/login` · `app/login/page.tsx` + `.login-*`(`globals.css:264-286`)
...
- **AppState:** `{ loaded, posts: Post[], nickname: string|null, avatar: string|null }`. (인증/신원은 `AppState`가 아니라 Supabase 세션 — `lib/auth.tsx`의 `useAuth`.)
...
- **저장 스키마:** `JSON.stringify({ posts, nickname, avatar })`. `state.loaded` 이후 모든 변경 시 저장(`useEffect([state])`). (인증 세션은 supabase-js가 별도 키로 관리.)
- **로드:** 최초 마운트에서 `localStorage.getItem(KEY)` → 있으면 파싱(`posts` 배열 검증, `nickname/avatar` 없으면 null), 없거나 파싱 실패 시 `seed()`로 시드.
