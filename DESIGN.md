...
#### 2.7.6b 여러 줄 필드 `.field-textarea` · `globals.css:503-510`
...
- **포커스 링:** `--shadow-focus`(`0 0 0 3px rgba(78,151,240,0.25)`) + `--border-focus`(#4e97f0). 적용: `.input-sm:focus-within`, `.field-input:focus`, `.field-textarea:focus`. (나머지 인풋/textarea는 `outline:none`이며 별도 포커스 링 없음 — `.detail-title`, `.detail-content`, `.composer input` 등.)
...
| `app/globals.css` 마이(468–532) | mypage-*/upload-btn/field-*(input/textarea/readonly)/saved-note/logout-card | §4.4, §2.7 |
