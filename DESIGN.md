...
### 2.10 FormatToolbar (플로팅 서식 툴바) · `components/editor/FormatToolbar.tsx` + `.fmt-*` (`globals.css:858-887`)

- **용도:** 본문 에디터에서 **텍스트를 범위 선택했을 때만** 선택 위에 뜨는 말풍선 서식 툴바(플로팅과 공존 — 상단 고정 툴바는 2026-07-23 스프린트에서 추가, §2.14). 마크 6종: 굵게·기울임·밑줄·취소선·인라인 코드·링크(`lib/editor/marks.ts`의 `MARKS`).
...
### 2.11 SlashMenu (에디터 슬래시 커맨드 메뉴) · `components/editor/SlashMenu.tsx` + `.slash2-*` (`globals.css:889-913`)
...
### 2.14 TopToolbar (상단 고정 툴바) · `components/editor/TopToolbar.tsx` + `.top-toolbar-*`(globals.css 끝 "Phase B: 상단 고정 툴바" 블록)

본문 에디터 위에 상시 노출되는 sticky 툴바(Phase B, 2026-07-23 스프린트) — 표(§2.15)·색(§2.16)·정렬(§5.14)까지 이미 등록된 확장을 전부 한 자리에서 쓸 수 있게 묶는 결승점 컴포넌트. 새 확장 등록 없이 기존 커맨드 체인만 호출한다.

- **용도:** 선택 없이도 항상 보이는 툴바 — **텍스트를 범위 선택했을 때만** 뜨는 플로팅 서식 툴바(§2.10)와 역할이 다르다(§2.10은 계속 공존, 서로 대체 관계 아님).
- **props:** `TopToolbar({ editor: Editor | null })`(null이면 렌더 안 함). 내용은 `TopToolbarContent({ editor })`로 분리(FormatToolbar·TableToolbar와 동일한 테스트 단위 관행).
- **해부(좌→우, `.top-toolbar__divider` 로 그룹 구분):**
  1. 텍스트 스타일 드롭다운 `<select class="top-toolbar__select" aria-label="텍스트 스타일">` — 일반/제목 1/제목 2/제목 3/인용 5옵션(콜아웃·토글은 슬래시 메뉴 §2.11 전용 — 여기서 다루지 않는다).
  1b. 글자 크기 드롭다운 `<select class="top-toolbar__select" aria-label="글자 크기">` — 기본/작게(13px)/크게(20px)/제목(28px). `FontSize`(TextStyle 전제, `lib/editor/marks.ts`)의 `setFontSize`/`unsetFontSize`(기본=unset). 현재 선택의 `textStyle.fontSize`가 옵션에 없으면 "기본"으로 표시.
  2. 마크 4종(B·I·U·S) — `.icon-btn`(§2.2) × 4, `MARK_ACTIONS`(`lib/editor/useFormatState.ts`)에서 인라인 코드(`code`)를 제외한 나머지(인라인 코드는 플로팅 서식 툴바 §2.10 전용으로 남긴다).
  3. 글자색·배경색 — `.icon-btn` × 2(`Baseline`/`Highlighter`, `aria-haspopup="true"`/`aria-expanded`) → 클릭 시 `.top-toolbar__popover-anchor` 안 `.top-toolbar__popover`에 `ColorPopover`(§2.16, `kind="text"|"highlight"`)를 토글 오픈. 스와치 클릭은 팝오버 자체 버블링으로 팝오버를 자동 닫는다. 링크 입력을 열면 색 팝오버가, 색 팝오버를 열면 링크 입력이 서로 닫힌다(상호 배제 — 같은 줄 팝오버 중첩 방지).
  4. 정렬 3종(왼쪽·가운데·오른쪽) — `setTextAlign(value)`(§5.14), 활성 상태는 `useFormatState`의 `align` 필드.
  5. 목록 3종(불릿·번호·체크박스) — `toggleList(type, itemType)`.
  6. 링크 — `.icon-btn`(`Link2`) → `.fmt-link-input`(§2.10과 동일 클래스·토큰 재사용) 미니 입력. Enter 적용, 빈 값(공백 포함) = 링크 해제, Esc 닫기, 열릴 때 자동 포커스 + 기존 href 미리 채움.
  7. 이미지 삽입 — `.icon-btn`(`ImageIcon`) → `openAttachmentPicker(editor.view, IMAGE_ACCEPT)`(§5.13, accept 값은 `lib/editor/insert.ts`의 비공개 `IMAGE_ACCEPT`와 동일 문자열을 로컬 상수로 복제).
  8. 표 삽입 — `.icon-btn`(`TableIcon`) → `insertTable({ rows: 3, cols: 3, withHeaderRow: true })`(§2.15).
  9. 실행취소·다시 실행 — `.icon-btn` × 2(`Undo2`/`Redo2`) → `editor.chain().focus().undo()/redo().run()`.
- **배치:** `<div class="top-toolbar-sticky">`(sticky 래퍼) → `<div class="top-toolbar" role="toolbar" aria-label="상단 툴바">`(내용). `position: sticky; top: 0; z-index: 10` — 슬래시 메뉴(`z-index: 30`, §2.11)·드래그 핸들(`z-index: 20`, §2.12)보다 낮게 잡아, 그 위에서 열리는 메뉴·핸들 메뉴가 항상 이 바보다 위에 보이게 한다.
- **스타일:** `.top-toolbar` = `display:flex; align-items:center; gap:4px; flex-wrap:wrap; padding:6px; background:--surface-page; border-bottom:1px solid --border-subtle`. 드롭다운 `.top-toolbar__select`는 §2.7.1 `.input-sm`과 동일 토큰(높이 32px, bg `--surface-subtle`, border `1px --border-subtle`, radius-md, font 13, `:focus` 시 bg `--surface-card` + `--border-focus` + `--shadow-focus`). 구분선 `.top-toolbar__divider` = 1×20px, bg `--border-subtle`.
- **상태(`.is-active`):** 마크·정렬·목록·링크 버튼은 `.icon-btn.is-active`(`aria-pressed`도 함께 부여) — bg `--surface-active`, color `--text-strong`(§2.10 `.fmt-bar`와 동일 값 재사용). **주의(B1 Minor 정리):** 기존 `.fmt-bar .icon-btn.is-active` 규칙(globals.css:867)은 `.fmt-bar` 안에서만 적용돼 상단 툴바에서는 하이라이트가 먹지 않는다 — `.top-toolbar .icon-btn.is-active` 규칙을 별도로 추가해 해결했다.
- **플로팅·슬래시·핸들과 공존:** 텍스트를 범위 선택하면 상단 툴바는 그대로 있고 플로팅 서식 툴바(§2.10)가 선택 위에 추가로 뜬다(대체가 아니라 동시 노출). `/`는 슬래시 메뉴(§2.11)를, 블록 hover는 드래그 핸들(§2.12)을 그대로 띄운다 — 세 UI 모두 상단 툴바의 표시 여부·z-index와 독립적으로 동작한다.
- **접근성:** 컨테이너 `role="toolbar" aria-label="상단 툴바"`. 마크·정렬·목록·링크 버튼은 `aria-pressed`(토글 시맨틱), 색 버튼은 `aria-haspopup="true"`+`aria-expanded`(디스클로저 시맨틱) — 표 삽입·이미지·undo/redo처럼 상태 없는 액션 버튼에는 둘 다 부여하지 않는다(TableToolbar §2.15 관행과 동일). 모든 버튼 `onMouseDown={(e)=>e.preventDefault()}`로 클릭 시 에디터 선택 영역·포커스를 유지한다.
- **예시:** `<TopToolbar editor={editor} />` — `PostEditor`에서 `EditorContent` 바로 위 형제로 렌더(결합부 불변, FormatToolbar·TableToolbar·SlashMenu·DragHandle과 동일 위치 관행).
...
글자색·배경색을 고르는 스와치 그리드 팝오버(C3). §1.1.6의 `TEXT_COLORS`/`HIGHLIGHT_COLORS`(`lib/editor/palette.ts`)를 그대로 렌더링해 소비하며, 트리거는 상단 툴바(§2.14)의 글자색·배경색 버튼이다.

- **props:** `ColorPopover({ editor: Editor; kind: "text" | "highlight" })`. `kind="text"` → `TEXT_COLORS`(글자색 8종), `kind="highlight"` → `HIGHLIGHT_COLORS`(배경색 8종).
- **해부:** `<div class="clr-pop" role="group" aria-label="글자색"|"배경색">` → `.clr-swatch`(button) × 8 — 색이 있는 항목은 `style={{ background: hex }}`, 각 배열의 첫 항목(`value: null`, "기본"/"없음")은 배경 없이 `.clr-swatch__none`(✕ 글리프)만 표시.
...
- **접근성:** 컨테이너 `role="group"`(스와치 8개가 목록 탐색 시맨틱을 요구하는 옵션 집합이 아니라 버튼 그룹이라 `listbox`+`option` 쌍 대신 채택 — 자식 button은 네이티브 button role 유지) + `aria-label`(종류별). 각 스와치는 `title`/`aria-label`이 색 이름 한국어 라벨(예: "빨강", "기본", "없음") — 색맹·스크린리더 사용자도 hex가 아닌 이름으로 식별한다.
- **예시:** `<ColorPopover editor={editor} kind="text" />` / `<ColorPopover editor={editor} kind="highlight" />` — 트리거는 상단 툴바(§2.14) `.top-toolbar__popover-anchor`/`.top-toolbar__popover`가 위치를 잡는다.
...
**페이지 템플릿(⑨) — `components/editor/TemplatePicker.tsx` + `lib/editor/templates.ts` + `.tpl-row`(`globals.css:915-925`):**
...
- **트리거:** A1/A2는 확장 등록·렌더 확인까지였고, 상단 툴바(Phase B, §2.14)가 왼쪽·가운데·오른쪽 3종 버튼으로 트리거를 결선했다(`justify`는 버튼 UI 없음 — 필요 시 추후 확장). 그 외에도 확장 기본 키보드 단축키(`Mod-Shift-L` 좌 / `Mod-Shift-E` 중앙 / `Mod-Shift-R` 우 / `Mod-Shift-J` 양쪽)와 `editor.commands.setTextAlign(...)` 프로그래밍 호출로 정렬을 바꿀 수 있다.
- **활성 상태 조회:** `useFormatState`(§2.10·§2.14 소비)의 `align` 필드가 `["left","center","right"]` 중 활성 값을 반환(`justify` 미포함 — 버튼 UI가 없어 필요 시 추후 확장). 문서 없는 경우 기본 `"left"`.
...
| 슬래시 메뉴 항목 라벨 | `BLOCKS` 레지스트리의 `label` 그대로(`lib/editor/blocks.ts`, 14종): `텍스트` `제목 1` `제목 2` `제목 3` `불릿 목록` `번호 목록` `체크박스 목록` `인용` `콜아웃` `토글` `구분선` `이미지` `파일` `표`(§2.15·§6.10 — 2026-07-23 스프린트 신설) |
| 슬래시 메뉴 빈 결과 | `결과 없음` |
...
### 6.10 표 · 상단 툴바 · 색/정렬 (2026-07-23 문서 작업 스프린트)

| 위치 | 문구 |
|---|---|
| 상단 툴바(§2.14) role=toolbar aria-label | `상단 툴바` |
| 텍스트 스타일 드롭다운 aria-label / 옵션 | `텍스트 스타일` / `일반` `제목 1` `제목 2` `제목 3` `인용` |
| 마크 버튼 title/aria-label(4종, §6.8 서식 툴바와 동일 문구 재사용) | `굵게` `기울임` `밑줄` `취소선` (인라인 코드는 상단 툴바에 없음 — 플로팅 서식 툴바 §2.10 전용) |
| 글자색·배경색 버튼 title/aria-label | `글자색` / `배경색` |
| 정렬 버튼 title/aria-label(3종) | `왼쪽 정렬` / `가운데 정렬` / `오른쪽 정렬` |
| 목록 버튼 title/aria-label(3종, §6.8 슬래시 메뉴 라벨과 동일 문구 재사용) | `불릿 목록` / `번호 목록` / `체크박스 목록` |
| 링크 버튼 title/aria-label | `링크` (미니 입력 placeholder/aria-label은 §6.8과 동일 `링크 주소 입력`) |
| 이미지 삽입 버튼 title/aria-label | `이미지 삽입` |
| 표 삽입 버튼 title/aria-label | `표 삽입` |
| undo/redo 버튼 title/aria-label | `실행취소` / `다시 실행` |
| 표 셀 플로팅 툴바(§2.15) role=toolbar aria-label | `표 편집` |
| 표 셀 툴바 버튼 title/aria-label(7종) | `행 위 삽입` / `행 아래 삽입` / `열 왼쪽 삽입` / `열 오른쪽 삽입` / `행 삭제` / `열 삭제` / `헤더 행 토글` |
| 색 팝오버(§2.16) role=group aria-label | `글자색` / `배경색`(종류별) |
| 글자색 스와치 이름(8종, `TEXT_COLORS`) | `기본` `회색` `빨강` `주황` `노랑` `초록` `파랑` `보라` |
| 배경색 스와치 이름(8종, `HIGHLIGHT_COLORS`) | `없음` `회색` `빨강` `주황` `노랑` `초록` `파랑` `보라`(글자색과 이름이 겹치지만 §1.1.6 각주대로 별개 hex — 이름은 색상 계열만 공유) |
