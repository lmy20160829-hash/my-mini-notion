...
출처: `app/globals.css` `:root {}`. **정의된 CSS 변수 총 74개** 전부를 아래에 열거한다. (Neutral 13 + Blue 8 + Accent 5 + Text 7 + Surface 7 + Border 4 + Accent-surface 5 + Status 3 + Danger-surface 1 + Avatar 2 + Font 2 + Radius 7 + Shadow 5 + Motion 5 = 74)
...
- **실제 사용된 lucide 아이콘 (25종)** 과 기본 size(px):
...
#### 2.7.2 상세 삭제 버튼 `.detail-delete-btn` · `globals.css:474-481`
...
#### 2.7.3 업로드 버튼 `.upload-btn` · `globals.css:511-518`
...
#### 2.7.4 필드 인풋 `.field-input` · `globals.css:523-529`
...
#### 2.7.5 읽기전용 필드 `.field-readonly` (+`__value`) · `globals.css:530-535`
...
#### 2.7.6 브랜드칩 `.brand-chip` · `globals.css:296-300` (+ 로그인 변형 273)
...
#### 2.7.7 컴포저 `.composer` (+`-wrap`/`__icon`/`-hint`) · `globals.css:363-375`
...
#### 2.7.8 슬래시 메뉴 `.slash-menu*` · `globals.css:377-403`
...
#### 2.7.9 글 카드 `.post-card*` · `globals.css:410-428`
...
#### 2.7.10 빈 상태 `.empty-state*` · `globals.css:440-451`
...
#### 2.7.13 스크롤바 `.mn-scroll` · `globals.css:189-192`
...
#### 2.7.14 글자 수 칩 `.detail-charcount` · `globals.css:496-502`
...
#### 2.7.15 커버 이미지 `.detail-cover*` · `globals.css:486-516` · `components/PostCover.tsx`
...
- **reduced-motion:** 커버 스켈레톤 shimmer에 한해 `@media (prefers-reduced-motion: reduce)`로 애니메이션을 끄고 정적 `--surface-hover` 배경으로 대체(§2.7.15). 그 배경도 토큰이므로 다크에서 자동으로 따라간다.
