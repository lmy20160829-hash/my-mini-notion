...
- **구성 순서:**
  ...
  5. `.sidebar__scroll`:
     ...
     - `SidebarSection label="앱"`: `SidebarItem 캘린더(Calendar)`, `할 일(SquareCheck)` — 둘은 **`disabled`(아직 만들지 않은 기능)**. `is-disabled` + `aria-disabled` + 툴팁 "…(준비 중)"(§2.5). 실제 구현 후보는 `docs/BACKLOG.md` 참조. `휴지통(Trash2)`은 **활성 항목** — `active={pathname === "/trash"}`, 클릭 시 `/trash`(§4.5)로 이동한다.
...
5개 화면: 로그인 `/login`, 업무 페이지(글 목록) `/`, 글 상세 `/posts/[id]`, 마이 페이지 `/mypage`, 휴지통 `/trash`.
...
| 섹션 "앱" + 항목 | `앱` / `캘린더` / `할 일` / `휴지통` (보이는 레이블). 캘린더·할 일 2종은 미구현 — 툴팁 `캘린더 (준비 중)` 등. 휴지통은 활성 — 툴팁 `휴지통`, `/trash` 이동(§3.3, §4.5) |
...
- **disabled:** 두 갈래다.
  ...
  - **아직 만들지 않은 셸 항목**(상단바 5종 + 사이드바 "앱"의 캘린더·할 일 2종, 총 7개)은 `aria-disabled` + `.is-disabled` — `cursor:default; opacity:0.45`, 툴팁에 "(준비 중)". `disabled` 속성을 쓰면 툴팁이 사라져 상태를 알릴 수단이 없어지기 때문이다(§2.2). (휴지통은 `/trash` 구현과 함께 활성화됐다 — §4.5.)
...
| `components/AppShell.tsx` | 셸 구성·네비(홈·휴지통)·인증 가드·검색·아이콘·사이드바 접기 토글·미구현 7종 비활성 표시 | §3, §3.3.1, §5.8, §1.7 |
...
3. 5개 화면 모두 포함, 빈 상태·슬래시 메뉴·저장 노트·삭제/영구 삭제 확인창까지 포함. ✅
