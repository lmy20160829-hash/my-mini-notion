...
wt2 ④ — `lib/editor/blocks.ts` 계약 13종 중 이미지·파일(wt3 소유)을 뺀 **11종**을
에디터 스키마에 등록한다. 시각 값은 전부 §1 토큰이며, 스타일은 globals.css **파일 끝**
`.blk-*` 조각만 쓴다(각 확장에 `HTMLAttributes.class`로 부여).
