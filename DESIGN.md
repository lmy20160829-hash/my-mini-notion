...
- **localStorage 키:** `"mini-notion-v1"` — **프로필 오버라이드 전용**.
  - **저장 스키마:** `JSON.stringify({ nickname, avatar })`. (게시글은 저장하지 않는다. 인증 세션은 supabase-js가 별도 키로 관리.)
...
| `lib/store.tsx` | 상태·세션 연동·낙관적 갱신·날짜포맷·상수 | §5, §6.6 |
...
4. 사용자 노출 한국어 문구 → §6에 화면별 전량 인용(placeholder 포함). ✅
