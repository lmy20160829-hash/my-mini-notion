...
출처: `app/globals.css` `:root {}`. **정의된 CSS 변수 총 75개** 전부를 아래에 열거한다. (Neutral 13 + Blue 8 + Accent 5 + Text 7 + Surface 7 + Border 4 + Accent-surface 5 + Status 4 + Danger-surface 1 + Avatar 2 + Font 2 + Radius 7 + Shadow 5 + Motion 5 = 75)
...
#### 1.1.1 Neutral 램프 — graphite (13색) · `globals.css:8-21`
...
`--text-on-accent`(#ffffff), `--border-focus`(#4e97f0), `--status-*` 4종, 프리미티브 램프 전체,
타이포·라운드·모션 토큰.
상태색은 다크 배경에서도 대비가 충분해 유지한다 — danger `#f0483e` 4.8:1, success `#2eb872` 6.7:1,
favorite `#ffbd18`(고대비). 구글 로고 4색은 브랜드 고정색이라 테마 무관.
...
- `--font-pretendard`: `app/layout.tsx`의 `next/font/local`이 주입.
  ...
  - 렌더링: `-webkit-font-smoothing: antialiased; text-rendering: optimizeLegibility;` (`globals.css:177-178`)
...
### 1.3 라운드 (Radius) · `globals.css:88-95`
...
### 1.4 그림자 (Shadow) · `globals.css:97-102` — 원문 그대로
...
### 1.5 모션 (Motion) · `globals.css:104-109, 127`
...
**키프레임 2 — `mnShimmer`** (`globals.css:504`): 커버 스켈레톤 로딩 표현(스피너 대체).
```css
...
### 2.1 Button · `components/ui/Button.tsx` + `.btn` (`globals.css:201-214`)
...
### 2.2 IconButton · `components/ui/IconButton.tsx` + `.icon-btn` (`globals.css:217-225`)
...
### 2.3 Badge · `components/ui/Badge.tsx` + `.badge` (`globals.css:228-232`)
...
### 2.4 Avatar · `components/ui/Avatar.tsx` + `.avatar` (`globals.css:235-240`)
...
### 2.5 SidebarItem · `components/ui/SidebarItem.tsx` + `.sidebar-item` (`globals.css:258-269`)
...
### 2.6 SidebarSection · `components/ui/SidebarSection.tsx` + `.sidebar-section*` (`globals.css:272-281`)
...
#### 2.7.1 검색 인풋 `.input-sm` · `globals.css:243-255`
...
#### 2.7.2 즐겨찾기 버튼 `.fav-btn` · `globals.css:430-438`
...
#### 2.7.3 상세 즐겨찾기 `.detail-fav-btn` · `globals.css:466-473`
- `30×30`, radius-md, color `--text-placeholder`; hover bg `--surface-hover`; `.is-fav` color `--gold-500`. 아이콘 `Star size={18}`.

#### 2.7.4 상세 삭제 버튼 `.detail-delete-btn` · `globals.css:474-481`
...
#### 2.7.5 업로드 버튼 `.upload-btn` · `globals.css:511-518`
...
#### 2.7.6 필드 인풋 `.field-input` · `globals.css:523-529`
...
#### 2.7.7 읽기전용 필드 `.field-readonly` (+`__value`) · `globals.css:530-535`
...
#### 2.7.8 브랜드칩 `.brand-chip` · `globals.css:296-300` (+ 로그인 변형 273)
...
#### 2.7.9 컴포저 `.composer` (+`-wrap`/`__icon`/`-hint`) · `globals.css:363-375`
...
#### 2.7.10 슬래시 메뉴 `.slash-menu*` · `globals.css:377-403`
...
#### 2.7.11 글 카드 `.post-card*` · `globals.css:410-428`
...
#### 2.7.12 빈 상태 `.empty-state*` · `globals.css:440-451`
...
#### 2.7.15 스크롤바 `.mn-scroll` · `globals.css:189-192`
...
#### 2.7.16 글자 수 칩 `.detail-charcount` · `globals.css:496-502`
...
#### 2.7.17 커버 이미지 `.detail-cover*` · `globals.css:486-516` · `components/PostCover.tsx`
...
출처: `components/AppShell.tsx` + `.app-root/.topbar/.app-body/.sidebar/.app-main` (`globals.css:286-330`). 라우트 그룹 `app/(app)/layout.tsx`가 `<AppShell>`로 모든 앱 화면을 감싼다.
...
### 4.1 로그인 `/login` · `app/login/page.tsx` + `.login-*`(`globals.css:335-357`)
...
### 4.2 업무 페이지(글 목록) `/` · `app/(app)/page.tsx` + `.list-*`(`globals.css:359-451`)
...
### 4.3 글 상세 `/posts/[id]` · `app/(app)/posts/[id]/page.tsx` + `.detail-*`(`globals.css:456-502`)
...
### 4.4 마이 페이지 `/mypage` · `app/(app)/mypage/page.tsx` + `.mypage*`(`globals.css:499-551`)
...
- **포커스 링:** `--shadow-focus` + `--border-focus`(#4e97f0, 두 테마 공통). 적용: `.input-sm:focus-within`, `.field-input:focus`. (나머지 인풋/textarea는 `outline:none`이며 별도 포커스 링 없음 — `.detail-title`, `.detail-content`, `.composer input` 등.)
...
- **커스텀 스크롤바:** `.mn-scroll`(→ [§2.7.15](#2715-스크롤바-mn-scroll)). thumb 11px, 색 `--border-default`/hover `--border-strong`, 트랙은 3px solid 페이지색 테두리로 여백감. 두 테마 모두 토큰이 따라간다. 네이티브 스크롤바는 `color-scheme`이 처리한다.
...
- **reduced-motion:** 커버 스켈레톤 shimmer에 한해 `@media (prefers-reduced-motion: reduce)`로 애니메이션을 끄고 정적 `--surface-hover` 배경으로 대체(§2.7.17). 그 배경도 토큰이므로 다크에서 자동으로 따라간다.
  > TODO(유지): reduced-motion은 여전히 커버 스켈레톤에만 적용됨. 다른 애니메이션 요소(`mnPop` 등)로 확장 시 동일 패턴을 적용해야 한다.
...
| 소스 파일 | 내용 | 반영 섹션 |
|---|---|---|
| `app/globals.css` `:root` | CSS 변수 75개(색/타이포/라운드/그림자/모션) + `color-scheme` | §1.1–§1.5 (전량 열거) |
...
1. `:root` CSS 변수 75개 → §1에 **전부** 열거(미사용 토큰은 "정의만" 표기). 다크 재정의 토큰은 §1.1.5에 전량 열거. ✅
2. `components/ui/` 파일 7개 = 문서 컴포넌트 7개(§2.1–2.6, §2.8) + CSS 전용 조각 15개(§2.7). 누락 없음. ✅
3. 4개 화면 모두 포함, 빈 상태·슬래시 메뉴·저장 노트·삭제 확인창까지 포함. ✅
4. 사용자 노출 한국어 문구 → §6에 화면별 전량 인용(시드/placeholder + 테마 토글 라벨 포함). ✅
...
> **미사용(정의만) 토큰 목록**(보존용): `--gray-25`, `--gray-700`(직접 참조 없음, 램프값으로만), `--blue-200`, `--blue-300`, `--blue-400`, `--gold-300`, `--accent-soft-fg`, `--status-warning`(및 그 참조원 `--amber-500`), `--radius-xs`, `--dur-slow`. — 삭제하지 말 것(디자인 시스템 원본에서 이식된 완전 램프/스케일의 일부).
