...
- **행 구성(`.tree-row`, flex·gap 2px):** 들여쓰기는 인라인 `padding-left: depth × 12px`(단계당 12px).
  ...
  2. **글 항목** — 기존 `.sidebar-item`(§2.5) 재사용(`flex:1; min-width:0; padding:0 8px 0 4px`로 재조정): `FileText size={16}` + 제목(폴백 "제목 없음", `title` 툴팁), 열린 글이면 `is-active`.
...
- **Post 타입:** `{ id: string; title: string; content: string; createdAt: number; deletedAt: number | null; contentDoc: EditorDoc | null; parentId: string | null }`. (`favorite` 없음 — 기능 제거. `deletedAt`은 소프트 삭제 시각 — null이면 살아 있는 글, 값이 있으면 휴지통에 있다. §4.5/§5.3. `parentId`는 페이지 중첩 §5.12 — null이면 루트.)
