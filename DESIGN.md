...
- **실제 사용된 lucide 아이콘 (26종)** 과 기본 size(px):
...
- **해부:** `<button class="sidebar-item[ is-active]" title={label}>` → `.sidebar-item__icon`(Icon 16) + `.sidebar-item__label`(말줄임) + (favorite면)`.sidebar-item__star`(`Star size={13}`).
...
- **레일 변형(`.sidebar.is-collapsed` 하위):** `width: 40px; padding: 0; justify-content: center` — 아이콘만 남고 `__label`·`__star`는 `display: none`. 높이·radius·상태 스타일은 동일.
- **상태별 스타일:**
...
사이드바는 **펼침**과 **접힘(아이콘 레일)** 두 상태를 가진다. 상태값은 스토어의 `sidebarCollapsed`(§5.6), 전환은 `toggleSidebar()`(§5.9).
...
- **AppState:** `{ loaded, loggedIn, posts: Post[], nickname: string|null, avatar: string|null, sidebarCollapsed: boolean }`.
- **파생값:** `displayName = state.nickname || "김민수"`, `email = "minsu.kim@gmail.com"`(상수).
- **localStorage 키:** `"mini-notion-v1"`.
- **저장 스키마:** `JSON.stringify({ posts, nickname, avatar, loggedIn, sidebarCollapsed })`. `state.loaded` 이후 모든 변경 시 저장(`useEffect([state])`).
- **로드:** 최초 마운트에서 `localStorage.getItem(KEY)` → 있으면 파싱(`posts` 배열 검증, `nickname/avatar` 없으면 null, `loggedIn`·`sidebarCollapsed` boolean 캐스팅), 없거나 파싱 실패 시 `seed()`로 시드.
  - `sidebarCollapsed`는 `!!d.sidebarCollapsed` — 이 필드가 없는 이전 저장 데이터는 기본값 `false`(펼침)로 읽힌다.
...
- 기본값 `false`(펼침). 값은 §5.6 스키마로 localStorage에 함께 저장되어 라우트 이동·새로고침 후에도 유지된다.
...
- **reduced-motion:** `@media (prefers-reduced-motion: reduce)` 대상 2건 — ① 커버 스켈레톤 shimmer는 애니메이션을 끄고 정적 `--surface-hover` 배경으로 대체(§2.7.17), ② `.sidebar { transition: none }`으로 접기/펼치기 폭 전환을 즉시 적용(§3.3.1). **다크모드**(`prefers-color-scheme`)는 여전히 미대응(라이트 단일 테마).
  > TODO: 다크모드는 현재 코드에 없음. 새 애니메이션 요소를 추가할 때 위 미디어 쿼리에 동일 패턴으로 등록할 것.
...
| `app/globals.css` App shell(219–259) | app-root/topbar/sidebar(+`is-collapsed` 레일·toolbar)/app-main | §3, §3.3.1 |
...
| `lib/store.tsx` | 상태·영속화·시드·날짜포맷·상수·사이드바 접힘 상태 | §5, §5.9, §6.6 |
| `03-reference-design.png` | ui:bowl 레퍼런스(다른 제품) | §0 레퍼런스 메모 |
| `02-prd.md`, `README.md` | 제품 맥락·원칙 | §0 |

### 자체 점검 결과

1. `:root` CSS 변수 74개(사이드바 폭 토큰 2개 추가) → §1에 **전부** 열거(미사용 토큰은 "정의만" 표기). ✅
