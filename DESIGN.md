...
출처: `app/globals.css` `:root {}` (라인 4–101). **정의된 CSS 변수 총 71개** 전부를 아래에 열거한다. (Neutral 13 + Blue 8 + Accent 5 + Text 7 + Surface 7 + Border 4 + Accent-surface 5 + Status 3 + Font 2 + Radius 7 + Shadow 5 + Motion 5 = 71)
...
```
- 적용: `.detail-cover__skeleton { animation: mnShimmer 1.4s ease-in-out infinite; }`(§2.7.15). `prefers-reduced-motion: reduce`에서는 `animation: none` + 정적 `--surface-hover` 배경(§7).
...
- **해부:** `<button class="sidebar-item[ is-active]">` → `.sidebar-item__icon`(Icon 16) + `.sidebar-item__label`(말줄임).
...
#### 2.7.2 상세 삭제 버튼 `.detail-delete-btn` · `globals.css:403-410`
...
#### 2.7.3 업로드 버튼 `.upload-btn` · `globals.css:440-447`
...
#### 2.7.4 필드 인풋 `.field-input` · `globals.css:452-458`
...
#### 2.7.5 읽기전용 필드 `.field-readonly` (+`__value`) · `globals.css:459-464`
...
#### 2.7.6 브랜드칩 `.brand-chip` · `globals.css:225-229` (+ 로그인 변형 273)
...
#### 2.7.7 컴포저 `.composer` (+`-wrap`/`__icon`/`-hint`) · `globals.css:292-304`
...
#### 2.7.8 슬래시 메뉴 `.slash-menu*` · `globals.css:306-332`
...
#### 2.7.9 글 카드 `.post-card*` · `globals.css:339-357`
...
#### 2.7.10 빈 상태 `.empty-state*` · `globals.css:369-380`
...
#### 2.7.13 스크롤바 `.mn-scroll` · `globals.css:122-125`
...
#### 2.7.14 글자 수 칩 `.detail-charcount` · `globals.css:425-431`
...
#### 2.7.15 커버 이미지 `.detail-cover*` · `globals.css:413-439` · `components/PostCover.tsx`
...
- **구성(위→아래):**
  ...
  2. **커버 이미지** `<PostCover key={post.id} />` → `.detail-cover`(§2.7.15), 브레드크럼과 제목 사이. 랜덤 고양이 사진(cataas). 로딩 시 스켈레톤(스피너 아님) → 이미지, 실패 시 중립 폴백. 장식 요소(`aria-hidden`), 편집 비방해(FR-007).
  ...
  6. **글자 수 칩** `<CharCount text={post.content} />` → `.detail-charcount`(§2.7.14), 본문 다음 마지막 자식. `post.content`만 전달(제목 제외, FR-008), grapheme 수를 우측 하단 sticky로 실시간 표시. 빈 내용 → `0자`.
- **상호작용:**
...
- **커스텀 스크롤바:** `.mn-scroll`(→ [§2.7.13](#2713-스크롤바-mn-scroll)). thumb 11px, 색 `--gray-200`/hover `--gray-300`, 트랙은 3px solid 페이지색 테두리로 여백감.
...
- **reduced-motion:** 커버 스켈레톤 shimmer에 한해 `@media (prefers-reduced-motion: reduce)`로 애니메이션을 끄고 정적 `--surface-hover` 배경으로 대체(`globals.css:434`, §2.7.15). 그 외 모션과 **다크모드**(`prefers-color-scheme`)는 여전히 미대응(라이트 단일 테마).
...
| `app/globals.css` Base(106–127) | reset, body, a, ::selection, placeholder, mn-scroll, mnPop | §1.5, §2.7.13, §7 |
...
1. `:root` CSS 변수 71개 → §1에 **전부** 열거(미사용 토큰은 "정의만" 표기). ✅
2. `components/ui/` 파일 6개 = 문서 컴포넌트 6개(§2.1–2.6) + CSS 전용 조각 13개(§2.7). 누락 없음. ✅
...
> **미사용(정의만) 토큰 목록**(보존용): `--gray-25`, `--gray-700`(직접 참조 없음, 램프값으로만), `--blue-200`, `--blue-300`, `--blue-400`, `--gold-300`, `--gold-500`, `--surface-inverse`, `--accent-soft-fg`, `--status-warning`(및 그 참조원 `--amber-500`), `--radius-xs`, `--dur-slow`. — 삭제하지 말 것(디자인 시스템 원본에서 이식된 완전 램프/스케일의 일부).
