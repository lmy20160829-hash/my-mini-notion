...
### 2.9 EventModal · `components/calendar/EventModal.tsx` + `.modal-*` (`globals.css:675-727`)
...
- **구성 순서:**
  ...
  5. `.sidebar__scroll`:
     ...
     - `SidebarSection label="앱"`: `SidebarItem 캘린더(Calendar)` — **구현됨**: `active={pathname === "/calendar"}`, 클릭 시 `/calendar` 이동(§4.5). 이어서 `할 일(SquareCheck)`, `휴지통(Trash2)` — 2종은 **`disabled`(아직 만들지 않은 기능)**. `is-disabled` + `aria-disabled` + 툴팁 "…(준비 중)"(§2.5). 실제 구현 후보는 `docs/BACKLOG.md` 참조.
...
5개 화면: 로그인 `/login`, 업무 페이지(글 목록) `/`, 글 상세 `/posts/[id]`, 마이 페이지 `/mypage`, 캘린더 `/calendar`.
...
### 4.5 캘린더 `/calendar` · `app/(app)/calendar/page.tsx` + `components/calendar/CalendarView.tsx` + `.calendar-*`(`globals.css:615-673`)
...
- **상호작용 상세:** → [§5.8](#58-캘린더-일정-crud-libeventsts--componentscalendar).
...
- **AppState:** `{ loaded, posts: Post[], nickname: string|null, profileImagePath: string|null, sidebarCollapsed: boolean }`. 스토어는 여기에 파생값 `profileImageUrl`(= 환경변수 앞부분 + `profileImagePath`)을 얹어 노출한다. (인증/신원은 `AppState`가 아니라 Supabase 세션 — `lib/auth.tsx`의 `useAuth`.)
...
- **localStorage 키:** `"mini-notion-v1"` — **별명 + UI 환경설정 전용**.
  - **저장 스키마:** `JSON.stringify({ nickname, sidebarCollapsed })`. (게시글·프로필 사진은 저장하지 않는다 — 서버가 원본이다. 인증 세션은 supabase-js가 별도 키로 관리.)
...
### 5.8 캘린더 일정 CRUD (`lib/events.ts` + `components/calendar/`)
...
| 섹션 빈 상태 | `아직 글이 없어요.` |
| 섹션 "앱" + 항목 | `앱` / `캘린더` / `할 일` / `휴지통` (보이는 레이블). 캘린더는 구현됨(툴팁 `캘린더`, `/calendar` 이동 — §4.5). 툴팁 `할 일 (준비 중)` 등 — 나머지 2종 미구현(§3.3) |
...
- **disabled:** 두 갈래다.
  ...
  - **아직 만들지 않은 셸 항목**(상단바 5종 + 사이드바 "앱" 2종 — 할 일·휴지통, 총 7개)은 `aria-disabled` + `.is-disabled` — `cursor:default; opacity:0.45`, 툴팁에 "(준비 중)". `disabled` 속성을 쓰면 툴팁이 사라져 상태를 알릴 수단이 없어지기 때문이다(§2.2). 캘린더는 구현되어 이 목록에서 빠졌다(§4.5).
...
- **reduced-motion:** `@media (prefers-reduced-motion: reduce)` 대상 3건 — ① 커버 스켈레톤 shimmer는 애니메이션을 끄고 정적 `--surface-hover` 배경으로 대체(§2.7.15, 그 배경도 토큰이라 다크에서 자동으로 따라간다), ② `.sidebar { transition: none }`으로 접기/펼치기 폭 전환을 즉시 적용(§3.3.1), ③ `.modal-card { animation: none }`으로 모달 등장 `mnPop`을 끈다(§2.9 — globals.css 끝의 별도 미디어 블록).
...
| `components/calendar/CalendarView.tsx` | 월 헤더·요일·42칸 그리드·칩·"+N"·모달 상태·월 범위 조회 | §4.5, §5.8 |
| `components/calendar/EventModal.tsx` | 일정 추가/수정 모달(필드·색상·검증·ESC/오버레이 닫기) | §2.9 |
| `app/(app)/calendar/page.tsx` | 캘린더 화면 진입점 | §4.5 |
| `lib/calendar.ts` | monthGrid(42칸)/eventsByDay/dateKey/datetime-local 변환 | §4.5, §5.8 |
| `lib/events.ts` | events 접근(조회/생성/수정/삭제, A2 0행 throw)·행↔CalendarEvent 매핑 | §5.8 |
...
| `components/AppShell.tsx` | 셸 구성·네비(캘린더 포함)·인증 가드·검색·아이콘·사이드바 접기 토글·미구현 7종 비활성 표시 | §3, §3.3.1, §1.7 |
...
3. 5개 화면 모두 포함, 빈 상태·슬래시 메뉴·저장 노트·삭제 확인창·일정 모달까지 포함. ✅
