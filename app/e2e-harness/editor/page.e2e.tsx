 ...
 * 이게 막아 주는 근거는 두 층이고, 강도가 다르니 구분해 둔다:
 *   1. Next CLI는 `build`/`start`에서 `NODE_ENV`가 **비어 있으면** "production"을
 *      넣는다(next/dist/bin/next:47,53-66). 다만 호출자가 명시로 다른 값을 넣으면
 *      경고만 하고 덮어쓰지 않는다 — 프레임워크의 보증이 아니라 기본값이다.
 *   2. 실제 보증은 배포 플랫폼 쪽이다. Vercel은 `NODE_ENV`를 시스템 예약 변수로 두어
 *      프로젝트 환경변수로 바꿀 수 없게 한다. 우리 배포 경로가 Vercel이므로,
 *      "Vercel 프로젝트 변수에 NEXT_PUBLIC_E2E=1이 설정되는 사고"는 이 조건에 걸린다.
 * 즉 **무조건**이 아니라 **우리 배포 경로에서는** 차단이다. 직접 `NODE_ENV=development`를
 * 내보내며 빌드하는 커스텀 스크립트를 만들면 뚫린다(현재 저장소엔 그런 경로가 없다).
 *
