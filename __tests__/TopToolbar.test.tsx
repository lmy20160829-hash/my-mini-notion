...
test("툴바에 스타일 드롭다운·마크·정렬·표·undo 버튼이 있다", () => {
  ...
  expect(screen.getByLabelText("글자 크기")).toBeDefined();
...
test("글자 크기 드롭다운에서 '크게'를 고르면 fontSize 20px가 적용된다", () => {
  const e = mk();
  e.chain().focus().insertContent("크기 테스트").selectAll().run();
  render(<TopToolbarContent editor={e} />);
  const select = screen.getByLabelText("글자 크기") as HTMLSelectElement;
  fireEvent.change(select, { target: { value: "20px" } });
  expect(e.getAttributes("textStyle").fontSize).toBe("20px");
  e.destroy();
});
