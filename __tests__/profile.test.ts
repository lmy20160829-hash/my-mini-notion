...
const noOverride = { nickname: null, imageUrl: null };
...
describe("mergeProfile", () => {
  ...
  test("로컬 nickname 오버라이드가 구글 이름보다 우선", () => {
    ...
    const p = mergeProfile(user, { nickname: "길동이", imageUrl: null });
    expect(p.displayName).toBe("길동이");
    expect(p.email).toBe("gildong@gmail.com");
  });

  test("업로드한 프로필 사진(imageUrl)이 구글 사진보다 우선", () => {
    const user = {
      email: "a@b.com",
      user_metadata: { avatar_url: "https://img/g.png" },
    };
    const p = mergeProfile(user, {
      nickname: null,
      imageUrl: "https://s/storage/v1/object/public/profile-image/uuid.png",
    });
    expect(p.avatarUrl).toBe(
      "https://s/storage/v1/object/public/profile-image/uuid.png"
    );
  });
  ...
  test("비로그인이라도 로컬 nickname 이 있으면 그 값을 쓴다", () => {
    expect(mergeProfile(null, { nickname: "익명", imageUrl: null }).displayName).toBe(
      "익명"
    );
  });
});
