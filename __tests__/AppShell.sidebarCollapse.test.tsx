...
async function renderShell() {
  // 상단바의 테마 토글이 useTheme을 쓰므로 ThemeProvider가 필요하다(실제 앱은 루트 레이아웃에서 감싼다).
  const result = render(
    <ThemeProvider>
      <AppProvider>
        <AppShell>
...
test("펼침 상태에서 토글 버튼은 '사이드바 접기'로 노출된다 (FR-001/FR-005)", async () => {
...
test("토글 클릭 시 사이드바가 레일로 접힌다 (US1-1/FR-002)", async () => {
  const { container } = await renderShell();
...
test("같은 버튼을 다시 클릭하면 원래 너비로 펼쳐진다 (US1-2/FR-003)", async () => {
  const { container } = await renderShell();
...
test("접힌 상태에서도 토글 버튼 1개가 보이며 '사이드바 펼치기'로 바뀐다 (US1-3/FR-004)", async () => {
  await renderShell();
...
test("연타해도 마지막 클릭의 최종 상태로 수렴한다 (Edge Case: 연타)", async () => {
  const { container } = await renderShell();
...
test("토글 버튼은 키보드로 포커스 가능하고 aria-controls로 사이드바를 가리킨다 (FR-012)", async () => {
  const { container } = await renderShell();
...
test("접힌 레일에서 검색·섹션 헤더·글 목록이 숨겨진다 (US2-3/FR-007)", async () => {
  await renderShell();
...
test("접힌 레일에 고정 내비 항목과 프로필 아바타가 남는다 (US2-3/FR-007)", async () => {
  const { container } = await renderShell();
  ...
  // 이름 자체는 반드시 남아 있어야 한다(FR-009). 캘린더·휴지통은 구현되어 접미사 없이 남는다.
...
test("접힌 레일의 홈 아이콘 클릭 시 홈으로 이동한다 (US2-1/FR-008)", async () => {
...
test("접힌 레일의 각 아이콘이 이름 툴팁을 가진다 (US2-2/FR-009)", async () => {
  const { container } = await renderShell();
...
test("접힌 레일에서도 홈의 활성 표시가 유지된다 (Edge Case: 활성 표시)", async () => {
  const { container } = await renderShell();
...
test("접은 뒤 재마운트해도 접힌 상태가 유지된다 (US3-2/SC-004)", async () => {
...
test("펼친 상태로 재마운트하면 펼친 상태가 유지된다 (US3-3)", async () => {
  const { container } = await renderShell();
