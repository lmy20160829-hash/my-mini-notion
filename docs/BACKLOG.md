...
## 상단 툴바 한 줄 정리 (UX 개선 후보)

**상태:** 후보 (2026-07-29 기록, E2E 스모크 스프린트 실측에서 발견 — 이번 스프린트
범위 제외, `TopToolbar.tsx`/`globals.css`는 건드리지 않았다)

**발견 경로:** E2E 스모크 #2(`e2e/toolbar-layout.spec.ts`) 작성 중 임계값을 실측하다가
발견했다. jsdom은 flex 레이아웃을 계산하지 않아 기존 65파일/523 vitest 테스트로는
이 상태를 볼 수 없었다 — Playwright로 실제 브라우저 좌표를 잰 뒤에야 드러났다.
...
**왜 UX 문제인가:** 실행취소/다시 실행은 편집 중 자주 쓰는 액션인데, 나머지 15개
버튼과 시각적으로 분리된 둘째 줄에 홀로 떨어져 있어 위치를 예측하기 어렵고, 툴바가
차지하는 세로 높이도 늘어나 본문 가시 영역이 줄어든다.

**해결 후보 (근본 해결):**

- **버튼 그룹화** — 관련 버튼을 묶어 차지 폭을 줄인다. 예: 정렬 3종(왼쪽/가운데/오른쪽)을
  드롭다운 하나로, 목록 3종(불릿/번호/체크박스)을 하나의 묶음 컨트롤로 합치면 아이콘
  버튼 개수 자체가 줄어 704px 안에 들어올 여지가 생긴다.
- **⋯ 더보기(overflow) 메뉴** — 사용 빈도가 낮은 버튼(예: 이미지 삽입, 표 삽입, 배경색)을
  `⋯` 오버플로 메뉴로 밀어 넣고, 굵게/기울임/정렬/실행취소처럼 우선순위 높은 버튼만
  상시 한 줄에 남긴다. `.top-toolbar__popover` 패턴(글자색·배경색 팝오버)을 재사용할 수
  있다.

gap(`4px`)·padding(`6px`)·구분선 폭 축소는 몇 px 여유를 벌 수는 있지만 버튼이 하나만
더 늘어도 다시 넘치는 **임시 완화책**일 뿐이다 — 근본 해결은 위 두 UX 개선(그룹화 또는
오버플로 메뉴) 쪽이다.

**착수 시 함께 볼 것:** `e2e/toolbar-layout.spec.ts`(현재 "두 줄까지 허용"으로 정직하게
개명해 둔 스모크)의 이름·임계값·주석을 착수 결과에 맞춰 다시 갱신해야 한다 — 한 줄로
고치면 `MAX_Y_SPREAD`/`MAX_TOOLBAR_HEIGHT`를 재실측해 좁혀야 회귀 감지력이 유지된다.
...
## ium.ai.kr 플랫폼 포털화 — 미니노션을 서브도메인 모듈로 (장기 비전)

**상태:** 장기 비전 (2026-07-27 기록). **우선순위는 현 스프린트·상단바 건 이후 논의** —
지금 당장 착수하지 않는다.

`ium.ai.kr`을 개인 플랫폼 포털로 만든다. 미니노션은 그 아래 **`notion.ium.ai.kr`** 서브도메인
모듈로 이전하고, 플랫폼 홈(포털 랜딩)은 이 저장소가 아니라 **별도 경량 프로젝트**로 둔다.

**방법 A — 서브도메인 분리(현재 검토 대상):** 미니노션은 지금 구조 그대로 `notion.` 아래로
옮기고, 루트 도메인에는 가벼운 포털 홈을 따로 배포한다. 저장소·배포 파이프라인이 분리돼
서로의 빌드를 막지 않는다는 게 장점.

**같이 검토할 것 — Supabase 공용 인증.** 서브도메인 간 로그인 상태를 공유할지 결정이 필요하다.
지금 `lib/supabase.ts`는 `persistSession: true` + PKCE라 세션이 **localStorage**에 들어가는데,
localStorage는 오리진 단위라 `ium.ai.kr`과 `notion.ium.ai.kr`이 서로 다른 저장소를 본다.
즉 지금 설정 그대로 서브도메인을 나누면 **각 서브도메인에서 따로 로그인**해야 한다.
한 번의 로그인을 공유하려면 세션 저장소를 부모 도메인 쿠키(`domain=.ium.ai.kr`)로 바꾸는
커스텀 storage 어댑터가 필요하고, 그러면 OAuth 리다이렉트 URL·CSRF·쿠키 크기까지 함께
봐야 한다. "공용 인증을 할 것인가"를 먼저 정하고 그 답에 따라 이 부분을 설계한다.

**연관:** 현재 배포는 이 저장소 `main` push가 트리거다(`.github/workflows/deploy.yml`은 GH
Pages 잔재로 죽어 있음 — 별도 정비 대상). 도메인 구조를 바꾸면 배포 트리거·환경변수
(`NEXT_PUBLIC_SUPABASE_*`)·OAuth 승인 리다이렉트 목록을 모두 함께 옮겨야 한다.
